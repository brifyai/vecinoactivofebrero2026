  ...
  return (
    <div className="p-8 space-y-8 animate-fade-in lg:ml-[400px]">
      <header className="mb-8 flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-white mb-2 flex items-center gap-3">Eventos Comunitarios <Calendar className="w-8 h-8 text-emerald-400" /></h1>
          <p className="text-gray-400">Participa en las actividades de tu barrio.</p>
        </div>
        <button onClick={handleNewEvent} className="bg-emerald-500 hover:bg-emerald-600 text-white px-6 py-2 rounded-xl font-medium transition-colors shadow-lg shadow-emerald-500/20">
          + Nuevo Evento
        </button>
      </header>
        ...
        {events.map((event) => (
          <Card key={event.id} className="group overflow-hidden p-0 border-white/10 hover:border-emerald-500/30">
            ...
            <div className="p-6 space-y-4">
              <h3 className="text-xl font-bold text-white group-hover:text-emerald-400 transition-colors">{event.title}</h3>
