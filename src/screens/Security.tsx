...
import { AlertTriangle, Shield, Phone, MapPin, ShieldCheck } from 'lucide-react';
...
export function Security() {
  ...
  const handleSOSAlert = () => {
    ...
    // TODO: Enviar alerta a todos los vecinos
  ...
  const handleAlertDetails = (alertId: number) => {
    ...
    // TODO: Abrir modal con detalles de la alerta
    if (alertData) {
  ...
  const handleEmergencyCall = (service: string, number: string) => {
    ...
    // TODO: Realizar llamada telefónica
  ...
  const handleQuadranteCall = () => {
    ...
    // TODO: Llamar al patrullero del cuadrante
  ...
  return (
    <div className="p-8 space-y-8 animate-fade-in lg:ml-[400px]">
      <header className="mb-8">
        <h1 className="text-3xl font-bold text-white mb-2 flex items-center gap-3">Seguridad Vecinal <ShieldCheck className="w-8 h-8 text-emerald-400" /></h1>
        <p className="text-gray-400">Reporta incidentes y mantén seguro tu barrio.</p>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          <Card className="bg-rose-900/20 border-rose-500/30">
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                <h2 className="text-xl font-bold text-white">¿Tienes una emergencia?</h2>
                <p className="text-gray-400">Presiona el botón para alertar a todos los vecinos.</p>
              </div>
            </div>
            <button onClick={handleSOSAlert} className="w-full py-4 bg-gradient-to-r from-rose-600 to-rose-500 hover:from-rose-500 hover:to-rose-400 text-white font-bold text-lg rounded-xl shadow-lg shadow-rose-500/20 transition-all transform hover:scale-[1.02]">
              ALERTA SOS
            </button>
          </Card>

          <Card>
            <h3 className="font-bold text-white mb-4">Alertas Recientes</h3>
            <div className="space-y-4">
              {alerts.map((alert) => (
                <div key={alert.id} className="p-4 bg-white/5 rounded-xl border border-white/5 flex justify-between items-start">
        ...
        <div className="space-y-6">
          <Card className="bg-gradient-to-br from-emerald-900/40 to-black border-emerald-500/20">
            <h3 className="font-bold text-white mb-4">Teléfonos de Emergencia</h3>
            <div className="space-y-3">
              <button onClick={() => handleEmergencyCall('Carabineros', '133')} className="w-full flex items-center justify-between p-3 bg-emerald-500/10 hover:bg-emerald-500/20 rounded-xl border border-emerald-500/10 transition-colors group">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-white font-medium">Carabineros</span>
                </div>
              ...
              <button onClick={() => handleEmergencyCall('Bomberos', '132')} className="w-full flex items-center justify-between p-3 bg-rose-500/10 hover:bg-rose-500/20 rounded-xl border border-rose-500/10 transition-colors group">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-white font-medium">Bomberos</span>
                </div>
              ...
              <button onClick={() => handleEmergencyCall('Ambulancia', '131')} className="w-full flex items-center justify-between p-3 bg-blue-500/10 hover:bg-blue-500/20 rounded-xl border border-blue-500/10 transition-colors group">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-white font-medium">Ambulancia</span>
                </div>
          ...
          <Card>
            <h3 className="font-bold text-white mb-2">Plan Cuadrante</h3>
            <p className="text-sm text-gray-400 mb-4">Contacta directamente al patrullero de tu zona.</p>
